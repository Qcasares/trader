...
> **Locally amended.** This file was installed from
> [`neondatabase/agent-skills`](https://github.com/neondatabase/agent-skills) and has since been
> edited for this repository. It therefore no longer matches the `computedHash` recorded in
> `skills-lock.json`, and that mismatch is the intended signal: the hash records upstream
> provenance, the file records what this repository actually sanctions. Do not run a blanket
> `skills update` over it. See "Updating Skills" below.
...
**Do not update these skills automatically, and never once per session.**

These files instruct an agent to run authenticated database and infrastructure commands. Anything
that can rewrite them can redirect those commands. `skills update` replaces the file contents and
the recorded hash with whatever upstream currently holds, so an upstream mistake or a compromised
account would reach this repository's database operations without anyone having read a diff. The
lock file pins a content hash of a mutable branch, not a reviewed revision, so it detects drift
after the fact rather than preventing it.

An update to a skill is a change to instructions this repository trusts, so treat it as a code
change:

1. Fetch the candidate into a scratch location, never over the installed copy.
2. Diff it against the file in the working tree and read the diff in full, paying particular
   attention to anything touching credentials, connection targets, migrations, or commands run
   without confirmation.
3. Apply it as an ordinary commit on a branch, and let review gate it.

Pin a tag or commit rather than a branch wherever the installing tool supports it. If it does not,
the diff review above is the only control, so do not skip it.
...
4. `neon env pull` — Fetches the current branch's Neon environment variables (`DATABASE_URL`, …) into your existing `.env`, or `.env.local` if you don't have one (override the target with `--file`). No branch ID needed; it reads `.neon`. `link` and `checkout` bundle this by default, but the bundled call can fail without failing the command, so in this repository run it explicitly. See [Branch-First Dev Flow](#branch-first-dev-flow).
...
- `neon env pull` — Fetches the current branch's Neon environment variables into your `.env` (see [Useful CLI Commands](#useful-cli-commands) above). `link` and `checkout` bundle this by default, but **run it as its own checked step here**, for the reason given below.
...
```bash
set -e
neon link --no-env-pull                     # once, pins project and branch in `.neon`
neon checkout dev-add-search --no-env-pull  # per feature, moves the branch pointer only
neon env pull                               # separate step, and it has to succeed
neon status                                 # confirm the resolved project and branch
```

**Do not rely on the `env pull` bundled into `link` and `checkout`.** Those commands update the
branch pointer in `.neon` whether or not their bundled env pull succeeds. A transient auth or
network failure therefore leaves the previous branch's `DATABASE_URL` sitting in `.env` while the
agent believes it has moved to an isolated branch. The next write does not fail, it succeeds
against the wrong database, and that database may be the one in production. A step that reports
success while leaving the connection target stale is not isolation, it is the appearance of it.

So pass `--no-env-pull`, run `neon env pull` as its own step under `set -e`, and confirm the
resolved project and branch with `neon status` before anything writes. In this repository that
means before `python -m src.db.migrate_cli`, before any `migrations/*.sql`, and before any job
that submits an order: all of them read `DATABASE_URL` and none of them re-check which branch it
points at.

As the agent, drive this loop yourself, and run the whole sequence between tasks rather than
`checkout` alone.
